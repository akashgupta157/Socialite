  ...
  return (
    ...
    >
      <div className="max-h-[82vh] overflow-scroll scrollbar-none md:max-h-[85vh]">
        ...
        {selectedFiles && (
          <div
            className={`max-h-[35vh] md:max-h-[40vh] overflow-y-scroll scrollbar-none w-full ${
              selectedFiles.length === 4 && "grid grid-cols-2 gap-2"
            } ${selectedFiles.length === 2 && "grid grid-cols-2 gap-2"} ${
          ...
          >
            {selectedFiles.map((file, index) => (
              <div key={index} className="relative mb-4">
                <div
                  className="absolute w-8 h-8 bg-[#78828e] bg-opacity-75 rounded-full flex items-center justify-center top-1 left-1 cursor-pointer"
      ...
      <div className="flex justify-between items-center border-t pt-2">
        ...
        <button
          ...
          className={`${
            input.length === 0
              ? "bg-[#4999df] cursor-not-allowed"
              : "bg-[#0381ec]"
          } text-white rounded-full py-2 px-4 text-sm font-bold`}
        >
          Post Now
        </button>
      </div>
