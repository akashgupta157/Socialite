...
const Profile = () => {
  ...
  const handlePost = async () => {
    ...
    try {
      const { data } = await axios.get(
        `/api/post?action=${currentTab}&userId=${userDetails._id}`,
        config
      );
      setPosts(data.posts);
    } catch (error) {
  ...
  useEffect(() => {
    const isUserFollowed = userDetails?.followers.some(
        ...
        <div className="max-h-[79vh] overflow-y-scroll scrollbar-none md:max-h-[90vh] md:w-[80%]">
          ...
          <nav className="md:hidden">
            <div className="flex justify-between items-center px-3 py-2">
              <div className="flex items-center gap-3">
                {userDetails && (
                  <ImageFallback
                    ...
                    height="0"
                    sizes="100vw"
                    className="rounded-full w-20 h-20 object-contain border"
                  />
                )}
                <div>
              ...
              {self ? (
                <button
                  className="bg-[#9d9290] text-white font-semibold px-2 py-1 rounded-lg"
                ...
                <button
                  ...
                  className={`${
                    isFollowing ? "bg-[#9d9290]" : "bg-[#0381ec]"
            ...
            <div className="border flex justify-evenly items-center mt-1">
                ...
                {userDetails && (
                  <>
                    <h1 className="font-bold">
                      {formatNumber(userDetails.posts.length)}
                    </h1>
                    <p>posts</p>
                  </>
                )}
              ...
              <div className="flex flex-col items-center">
                {userDetails && (
                  <>
                    <h1 className="font-bold">
                      {formatNumber(userDetails.followers.length)}
                    </h1>
                    <p>followers</p>
                  </>
                )}
              ...
              <div className="flex flex-col items-center">
                {userDetails && (
                  <>
                    <h1 className="font-bold">
                      {formatNumber(userDetails.following.length)}
                    </h1>
                    <p>following</p>
                  </>
                )}
            ...
            <div className="flex justify-between items-center gap-10">
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-16">
                  ...
                  {self ? (
                    <button
                      className="bg-[#9d9290] text-white font-semibold px-5 py-1 rounded-lg"
                      onClick={() => setOpenModal(true)}
                    >
                      Edit Profile
                    </button>
                    ...
                    <button
                      ...
                      className={`${
                        isFollowing ? "bg-[#9d9290]" : "bg-[#0381ec]"
                      }  text-white font-semibold px-5 py-1 rounded-lg`}
                    >
                      {isFollowing ? "Following" : "Follow"}
                    </button>
                  )}
                ...
                <div className="flex items-center gap-5">
                  {userDetails && (
                    <h1 className="font-bold">
                      {formatNumber(userDetails.posts.length)} posts
                    </h1>
                  )}
                  {userDetails && (
                    <h1 className="font-bold">
                      {formatNumber(userDetails.followers.length)} followers
                    </h1>
                  )}
                  {userDetails && (
                    <h1 className="font-bold">
                      {formatNumber(userDetails.following.length)} following
                    </h1>
                  )}
                </div>
          ...
          <div className="flex justify-evenly items-center">
            <button
              className={`px-2 md:text-lg md:font-semibold ${
                currentTab === "posts" &&
                "text-[#0381ec] border-t-2 border-[#0381ec]"
              }`}
        ...
        <Modal.Body>
          <div className="relative max-w-fit mb-2">
            <ImageFallback
              ...
              height="0"
              sizes="100vw"
              className="rounded-full w-28 h-28 object-contain border"
            />
            <label
              htmlFor="file"
              className="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] text-white cursor-pointer bg-black p-3 rounded-full bg-opacity-[0.5] hover:bg-opacity-[0.4]"
            >
              <Camera />
            </label>
            <input
